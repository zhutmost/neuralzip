...
Proper ResNet-s for CIFAR10 (for fair comparison etc.) has the following
...
class PaddingShortcut(nn.Module):
    def __init__(self, planes):
        super().__init__()
        self.planes = planes

    def forward(self, x):
        return F.pad(x[:, :, ::2, ::2], [0, 0, 0, 0, self.planes // 4, self.planes // 4])
    ...
    def __init__(self, in_planes, planes, stride=1, option='A'):
        ...
        if stride != 1 or in_planes != planes:
            if option == 'A':
                ...
                self.shortcut = PaddingShortcut(planes)
            elif option == 'B':
                self.shortcut = nn.Sequential(
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        s = load_state_dict_from_url(model_urls[arch], progress=progress, map_location=lambda storage, loc: storage)
